...
public void draw() {
  ...
  for (int i = 0; i < shots.size(); i++){
    ...
    for (int j = rockArr.size() - 1; j >= 0; j--){
      float astBullDist = dist((float) shots.get(j).getCenterX(), (float) shots.get(j).getCenterY(), (float) rockArr.get(j).getRockCenterX(), (float) rockArr.get(j).getRockCenterY());
